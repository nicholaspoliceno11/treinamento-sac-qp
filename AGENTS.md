...
- Recommended (live reload): `docsify serve . --port 3000` then open `http://localhost:3000`.
 ...
 - Because a custom npm `prefix` is set, shells print a harmless `nvm ... incompatible with nvm` warning on startup. It is cosmetic — `node` and `docsify` still resolve correctly; ignore it.
